...
\centering
\begin{tikzpicture}[
    flowbox/.style={draw, rounded corners, align=center, fill=blue!6, text width=0.17\textwidth, minimum height=1.55cm, inner sep=5pt},
    tablebox/.style={draw, rounded corners, align=left, fill=green!8, text width=0.235\textwidth, minimum height=2.05cm, inner sep=6pt},
    outbox/.style={draw, rounded corners, align=center, fill=orange!10, text width=0.17\textwidth, minimum height=1.55cm, inner sep=5pt},
    arrow/.style={-Latex, thick},
    node distance=0.28cm
]
\node[flowbox] (label) {YOLO output\\class label};
\node[tablebox, right=of label] (map) {\textbf{yolo\_mappings}\\YOLO class\\matched food name\\match score};
\node[tablebox, right=of map] (foods) {\textbf{indb\_foods}\\food name\\calories\\protein, carbs, fat\\source};
\node[outbox, right=of foods] (response) {Nutrition\\response\\for frontend};

\draw[arrow] (label) -- (map);
\draw[arrow] (map) -- node[above, font=\small]{matched name} (foods);
...
The API design exposes endpoints for health checks, image analysis, nutrition testing, nutrition validation, macro calculation, goal metadata, health-condition metadata, and recommendations. Table \ref{tab:api-design} lists the implemented endpoints.
