...
class VisionService:
    ...
    async def initialize(self):
        try:
            ...
            model_path_env = str(self.model_path or Settings.from_environment().model_path)
    ...
    def analyze_image(self, image_content: bytes, conf: float = 0.25, iou: float = 0.45) -> dict | None:
        ...
        try:
            ...
            for box, class_name, confidence in valid_boxes:
                ...
                detections.append({"food_label": class_name, "confidence": round(confidence, 4), "bounding_box": bbox})

            logger.info(f"Detected {len(detections)} foods: {[d['food_label'] for d in detections]}")

            return {"detections": detections, "img_width": img.width, "img_height": img.height}
